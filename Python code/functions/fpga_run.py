...
def send_p(p): pass  # Hook placeholder
...
def fpga_run(p):
    ...
    while len(data) == 0:
        # Launch read listener into background via subprocess
        if os.name == 'nt':
            # Run command piping natively using Windows Command Prompt syntax wrapper
            subprocess.Popen('cat \\\\.\\xillybus_read_32 > data_rx.bin', shell=True)
        
        # Construct parameters message buffer
        ...
        # Bit stuffing and flipping dimension matrix setup
        M = np.fliplr(M)
        ...
        # Force terminate orphan cmd shells 
        if os.name == 'nt':
    ...
    # Unpack read data packets layout logic
    ...
    # Cleanup / Reset back to Idle state loop configuration commands
